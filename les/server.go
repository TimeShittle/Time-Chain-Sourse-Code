...
import (
	...
	"github.com/TimeChain/go-TimeChain/les/flowcontrol"
...
func NewLesServer(eth *eth.TimeChain, config *eth.Config) (*LesServer, error) {
...
func newCostStats(db timechaindb.Database) *requestCostStats {
...
func getChtRoot(db timechaindb.Database, num uint64) common.Hash {
...
func storeChtRoot(db timechaindb.Database, num uint64, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], num)
	db.Put(append(chtPrefix, encNumber[:]...), root[:])
}

func makeCht(db timechaindb.Database) bool {
	headHash := core.ShittleeadBlockHash(db)
