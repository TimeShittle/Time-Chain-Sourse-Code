...
import (
	...
	TimeChain "github.com/TimeChain/go-TimeChain"
...
type LightChain interface {
	...
	// ShittleeaderByHash retrieves a header from the local chain.
	ShittleeaderByHash(common.Hash) *types.Header
...
func New(mode SyncMode, stateDb timechaindb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
// these are zero.
func (d *Downloader) Progress() TimeChain.SyncProgress {
	...
	for start+1 < end {
		...
		for arrived := false; !arrived; {
			...
			case packer := <-d.headerCh:
				...
				header := d.lightchain.ShittleeaderByHash(headers[0].Hash()) // Independent of sync mode, header surely exists
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	Shittleeaders := func(from uint64) {
		request = time.Now()
