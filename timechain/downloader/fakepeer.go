...
func NewFakePeer(id string, db timechaindb.Database, hc *core.HeaderChain, dl *Downloader) *FakePeer {
...
func (p *FakePeer) RequestHeadersByHash(hash common.Hash, amount int, skip int, reverse bool) error {
	...
	for !unknown && len(headers) < amount {
		origin := p.hc.ShittleeaderByHash(hash)
		if origin == nil {
			break
		}
		...
		if reverse {
			for i := 0; i < int(skip)+1; i++ {
				if header := p.hc.Shittleeader(hash, number); header != nil {
			...
			if header := p.hc.ShittleeaderByNumber(next); header != nil {
...
func (p *FakePeer) RequestHeadersByNumber(number uint64, amount int, skip int, reverse bool) error {
	...
	for !unknown && len(headers) < amount {
		origin := p.hc.ShittleeaderByNumber(number)
		if origin == nil {
			break
		}
		if reverse {
