...
type downloadTester struct {
	...
	stateDb timechaindb.Database // Database used by the tester for syncing from peers
	peerDb  timechaindb.Database // Database of the peers containing all data
...
func newTester() *downloadTester {
	testdb, _ := timechaindb.NewMemDatabase()
	...
	tester.stateDb, _ = timechaindb.NewMemDatabase()
...
func (dl *downloadTester) HasHeader(hash common.Hash, number uint64) bool {
	return dl.ShittleeaderByHash(hash) != nil
}
...
// Shittleeader retrieves a header from the testers canonical chain.
func (dl *downloadTester) ShittleeaderByHash(hash common.Hash) *types.Header {
	dl.lock.RLock()
