...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"TimeChainStats", "explorer", "wallet", "faucet"} {
	...
	// If we have TimeChainStats running, ask whether to make the secret public or not
	var TimeChainStats bool
	if w.conf.TimeChainStats != "" {
		fmt.Println()
		fmt.Println("Include TimeChainStats secret on dashboard (y/n)? (default = yes)")
		TimeChainStats = w.readDefaultString("y") == "y"
	}
	// Try to deploy the dashboard container on the host
	if out, err := deployDashboard(client, w.network, infos.port, infos.host, listing, &w.conf, TimeChainStats); err != nil {
