...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.TimeChainStats == "" {
		log.Error("No TimeChainStats server configured")
		return
	}
	...
	fmt.Println()
	if infos.TimeChainStats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.TimeChainStats = w.readString() + ":" + w.conf.TimeChainStats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.TimeChainStats)
		infos.TimeChainStats = w.readDefaultString(infos.TimeChainStats) + ":" + w.conf.TimeChainStats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy TimeChain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
