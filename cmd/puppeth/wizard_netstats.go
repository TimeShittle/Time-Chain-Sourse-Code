...
func (w *wizard) networkStats(tips bool) {
	...
	for server, pubkey := range w.conf.Servers {
		...
		logger.Debug("Checking for TimeChainStats availability")
		if infos, err := checkTimeChainStats(client, w.network); err != nil {
			if err != ErrServiceUnknown {
				services["TimeChainStats"] = err.Error()
			}
		} else {
			services["TimeChainStats"] = infos.String()
			protips.TimeChainStats = infos.config
		}
	...
	if protips.TimeChainStats != "" {
		w.conf.TimeChainStats = protips.TimeChainStats
	}
...
func (p *protips) print(network string) {
	...
	fullinit, lightinit := "", ""
	if p.genesis != "" {
		fullinit = fmt.Sprintf("Shittle --datadir=$HOME/.%s init %s.json && ", network, network)
		lightinit = fmt.Sprintf("Shittle --datadir=$HOME/.%s --light init %s.json && ", network, network)
	}
	// If an TimeChainStats server is available, add the TimeChainStats flag
	statsflag := ""
	if p.TimeChainStats != "" {
		if strings.Contains(p.TimeChainStats, " ") {
			statsflag = fmt.Sprintf(` --TimeChainStats="yournode:%s"`, p.TimeChainStats)
		} else {
			statsflag = fmt.Sprintf(` --TimeChainStats=yournode:%s`, p.TimeChainStats)
		}
	}
	...
	tips = append(tips, fmt.Sprintf("%sShittle --networkid=%d --datadir=$HOME/.%s --cache=1024%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a full node with recent data only")
	tips = append(tips, fmt.Sprintf("%sShittle --networkid=%d --datadir=$HOME/.%s --cache=512 --fast%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a light node with on demand retrievals")
	tips = append(tips, fmt.Sprintf("%sShittle --networkid=%d --datadir=$HOME/.%s --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))

	tasks = append(tasks, "Run an embedded node with constrained memory")
	tips = append(tips, fmt.Sprintf("%sShittle --networkid=%d --datadir=$HOME/.%s --cache=32 --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))
