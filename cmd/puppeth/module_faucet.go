...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/TimeChain                                                            && \
	(cd $GOPATH/src/github.com/TimeChain && git clone --depth=1 https://github.com/TimeChain/go-TimeChain) && \
  go build -v github.com/TimeChain/go-TimeChain/cmd/faucet                                              && \
...
CMD [ \
	"/faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--TimeChainStats", "{{.TimeChainStats}}", "--TimeChainport", "{{.TimeChainport}}", \
...
services:
  faucet:
    ...
    ports:
      - "{{.TimeChainport}}:{{.TimeChainport}}"{{if not .VHost}}
      - "{{.ApiPort}}:8080"{{end}}
    volumes:
      - {{.Datadir}}:/root/.faucet
    environment:
      - ETH_PORT={{.TimeChainport}}
      - ETH_NAME={{.EthName}}
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"TimeChainStats":      config.node.TimeChainStats,
		"TimeChainport":       config.node.portFull,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"TimeChainport":       config.node.portFull,
		"EthName":       config.node.TimeChainStats[:strings.Index(config.node.TimeChainStats, ":")],
		"GitHubUser":    config.githubUser,
		"GitHubToken":   config.githubToken,
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
...
func (info *faucetInfos) String() string {
	return fmt.Sprintf("host=%s, api=%d, eth=%d, amount=%d, minutes=%d, tiers=%d, github=%s, captcha=%v, TimeChainStats=%s", info.host, info.port, info.node.portFull, info.amount, info.minutes, info.tiers, info.githubUser, info.captchaToken != "", info.node.TimeChainStats)
}
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			TimeChainStats: infos.envvars["ETH_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
