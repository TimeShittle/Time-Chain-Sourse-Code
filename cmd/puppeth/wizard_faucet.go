...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Timer to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.TimeChainStats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.TimeChainStats = w.readString() + ":" + w.conf.TimeChainStats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.TimeChainStats)
		infos.node.TimeChainStats = w.readDefaultString(infos.node.TimeChainStats) + ":" + w.conf.TimeChainStats
	}
