...
import (
	...
	"TimeShittle.org/x/crypto/ssh/terminal"
)
...
type config struct {
	...
	TimeChainStats  string        // TimeChainStats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // TimeChain services known to be running on servers
...
// converts it to an TimeChain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by TimeChainStats, which compares IPs textually, not by value.
