...
import (
	...
	"github.com/TimeChain/go-TimeChain/cmd/utils"
	"github.com/TimeChain/go-TimeChain/common"
	"github.com/TimeChain/go-TimeChain/console"
	"github.com/TimeChain/go-TimeChain/contracts/ens"
	"github.com/TimeChain/go-TimeChain/crypto"
	"github.com/TimeChain/go-TimeChain/TimeChainClient"
	"github.com/TimeChain/go-TimeChain/internal/debug"
	...
	"github.com/TimeChain/go-TimeChain/swarm"
	bzzapi "github.com/TimeChain/go-TimeChain/swarm/api"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:  "swap-api",
		Usage: "URL of the TimeChain API provider to use to settle SWAP payments",
	}
	...
	EnsAPIFlag = cli.StringFlag{
		Name:  "ens-api",
		Usage: "URL of the TimeChain API provider to use for ENS record lookups",
		Value: node.DefaultIPCEndpoint("Shittle"),
	}
...
// This init function sets defaults so cmd/swarm can run alongside Shittle.
func init() {
...
var app = utils.NewApp(gitCommit, "TimeChain Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The go-TimeChain Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.TimeChain/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.TimeChain/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.TimeChain/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.TimeChain/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := TimeChainClient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
...
func registerBzzService(ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var swapClient *TimeChainClient.Client
		if swapapi != "" {
			log.Info("connecting to SWAP API", "url", swapapi)
			swapClient, err = TimeChainClient.Dial(swapapi)
			if err != nil {
				return nil, fmt.Errorf("error connecting to SWAP API %s: %s", swapapi, err)
			}
		}

		var ensClient *TimeChainClient.Client
		if ensapi != "" {
			...
			ensClient = TimeChainClient.NewClient(client)
