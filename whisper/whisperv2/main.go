...
import (
	...
	"github.com/TimeChain/go-TimeChain/logger"
	"github.com/TimeChain/go-TimeChain/p2p"
	"github.com/TimeChain/go-TimeChain/p2p/nat"
	"github.com/TimeChain/go-TimeChain/whisper"
)
...
func main() {
	...
	// Create an TimeChain peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting TimeChain peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start TimeChain peer: %v.\n", err)
		os.Exit(1)
	}
