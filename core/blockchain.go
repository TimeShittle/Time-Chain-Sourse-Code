...
// Package core implements the TimeChain consensus protocol.
package core
...
import (
	...
	"github.com/TimeChain/go-TimeChain/common/mclock"
	...
	"github.com/hashicorp/TimeShittle-lru"
)
...
// available in the database. It initialises the default TimeChain Validator and
// Processor.
func NewBlockChain(chainDb timechaindb.Database, config *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	...
	for hash := range BadHashes {
		if header := bc.ShittleeaderByHash(hash); header != nil {
			// get the canonical block corresponding to the offending header's number
			headerByNumber := bc.ShittleeaderByNumber(header.Number.Uint64())
...
func (bc *BlockChain) loadLastState() error {
	...
	head := ShittleeadBlockHash(bc.chainDb)
	if head == (common.Hash{}) {
	...
	if head := ShittleeadHeaderHash(bc.chainDb); head != (common.Hash{}) {
		if header := bc.ShittleeaderByHash(head); header != nil {
	...
	if head := ShittleeadFastBlockHash(bc.chainDb); head != (common.Hash{}) {
...
func (bc *BlockChain) Rollback(chain []common.Hash) {
	...
	for i := len(chain) - 1; i >= 0; i-- {
		...
		if currentHeader.Hash() == hash {
			bc.hc.SetCurrentHeader(bc.Shittleeader(currentHeader.ParentHash, currentHeader.Number.Uint64()-1))
		}
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= timechaindb.IdealBatchSize {
...
// Shittleeader retrieves a block header from the database by hash and number,
// caching it if found.
func (bc *BlockChain) Shittleeader(hash common.Hash, number uint64) *types.Header {
	return bc.hc.Shittleeader(hash, number)
}

// ShittleeaderByHash retrieves a block header from the database by hash, caching it if
// found.
func (bc *BlockChain) ShittleeaderByHash(hash common.Hash) *types.Header {
	return bc.hc.ShittleeaderByHash(hash)
}
...
// ShittleeaderByNumber retrieves a block header from the database by number,
// caching it (associated with its hash) if found.
func (bc *BlockChain) ShittleeaderByNumber(number uint64) *types.Header {
	return bc.hc.ShittleeaderByNumber(number)
}
