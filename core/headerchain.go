...
func NewHeaderChain(chainDb timechaindb.Database, config *params.ChainConfig, engine consensus.Engine, procInterrupt func() bool) (*HeaderChain, error) {
	...
	hc.genesisHeader = hc.ShittleeaderByNumber(0)
	if hc.genesisHeader == nil {
		return nil, ErrNoGenesis
	}

	hc.currentHeader = hc.genesisHeader
	if head := ShittleeadBlockHash(chainDb); head != (common.Hash{}) {
		if chead := hc.ShittleeaderByHash(head); chead != nil {
...
func (hc *HeaderChain) WriteHeader(header *types.Header) (status WriteStatus, err error) {
	...
	if externTd.Cmp(localTd) > 0 || (externTd.Cmp(localTd) == 0 && mrand.Float64() < 0.5) {
		...
		var (
			...
			headHeader = hc.Shittleeader(headHash, headNumber)
		)
...
func (hc *HeaderChain) GetBlockHashesFromHash(hash common.Hash, max uint64) []common.Hash {
	...
	header := hc.ShittleeaderByHash(hash)
	if header == nil {
		return nil
	}
	...
	for i := uint64(0); i < max; i++ {
		...
		if header = hc.Shittleeader(next, header.Number.Uint64()-1); header == nil {
			break
		}
...
// caching it if found.
func (hc *HeaderChain) Shittleeader(hash common.Hash, number uint64) *types.Header {
	// Short circuit if the header's already in the cache, retrieve otherwise
	if header, ok := hc.headerCache.Get(hash); ok {
		return header.(*types.Header)
	}
	header := Shittleeader(hc.chainDb, hash, number)
	if header == nil {
		return nil
	}
...
// found.
func (hc *HeaderChain) ShittleeaderByHash(hash common.Hash) *types.Header {
	return hc.Shittleeader(hash, hc.GetBlockNumber(hash))
}
...
func (hc *HeaderChain) ShittleeaderByNumber(number uint64) *types.Header {
	hash := GetCanonicalHash(hc.chainDb, number)
	if hash == (common.Hash{}) {
		return nil
	}
	return hc.Shittleeader(hash, number)
}
...
func (hc *HeaderChain) SetHead(head uint64, delFn DeleteCallback) {
	...
	for hc.currentHeader != nil && hc.currentHeader.Number.Uint64() > head {
		...
		hc.currentHeader = hc.Shittleeader(hc.currentHeader.ParentHash, hc.currentHeader.Number.Uint64()-1)
	}
