...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	if full {
		...
		hash1 = blockchain.ShittleeaderByNumber(uint64(i)).Hash()
		hash2 = blockchain2.ShittleeaderByNumber(uint64(i)).Hash()
	}
	if hash1 != hash2 {
...
func TestLastBlock(t *testing.T) {
	...
	bchain.insert(block)
	if block.Hash() != ShittleeadBlockHash(bchain.chainDb) {
...
func testReorg(t *testing.T, first, second []int, td int64, full bool) {
	...
	if full {
		...
		for header := bc.ShittleeaderByNumber(bc.CurrentHeader().Number.Uint64() - 1); header.Number.Uint64() != 0; prev, header = header, bc.ShittleeaderByNumber(header.Number.Uint64()-1) {
	...
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		for j := 0; j < i-failAt; j++ {
			if full {
				...
				if header := blockchain.ShittleeaderByNumber(failNum + uint64(j)); header != nil {
...
func TestFastVsFullChains(t *testing.T) {
	...
	var (
		gendb, _ = timechaindb.NewMemDatabase()
	...
	archiveDb, _ := timechaindb.NewMemDatabase()
	...
	fastDb, _ := timechaindb.NewMemDatabase()
	...
	for i := 0; i < len(blocks); i++ {
		...
		if fheader, aheader := fast.ShittleeaderByHash(hash), archive.ShittleeaderByHash(hash); fheader.Hash() != aheader.Hash() {
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	lightDb, _ := timechaindb.NewMemDatabase()
