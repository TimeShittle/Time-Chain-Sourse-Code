...
func TestHeaderStorage(t *testing.T) {
	...
	if entry := Shittleeader(db, header.Hash(), header.Number.Uint64()); entry != nil {
	...
	if entry := ShittleeaderRLP(db, header.Hash(), header.Number.Uint64()); entry == nil {
...
func TestBlockStorage(t *testing.T) {
	...
	if entry := Shittleeader(db, block.Hash(), block.NumberU64()); entry != nil {
		t.Fatalf("Non existent header returned: %v", entry)
	}
...
func TestHeadStorage(t *testing.T) {
	...
	if entry := ShittleeadHeaderHash(db); entry != (common.Hash{}) {
		t.Fatalf("Non head header entry returned: %v", entry)
	}
	if entry := ShittleeadBlockHash(db); entry != (common.Hash{}) {
		t.Fatalf("Non head block entry returned: %v", entry)
	}
	if entry := ShittleeadFastBlockHash(db); entry != (common.Hash{}) {
	...
	if entry := ShittleeadHeaderHash(db); entry != blockHead.Hash() {
		t.Fatalf("Head header hash mismatch: have %v, want %v", entry, blockHead.Hash())
	}
	if entry := ShittleeadBlockHash(db); entry != blockFull.Hash() {
		t.Fatalf("Head block hash mismatch: have %v, want %v", entry, blockFull.Hash())
	}
	if entry := ShittleeadFastBlockHash(db); entry != blockFast.Hash() {
