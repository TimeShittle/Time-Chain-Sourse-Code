...
type ChainIndexer struct {
	chainDb  timechaindb.Database      // Chain database to index the data from
	indexDb  timechaindb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb, indexDb timechaindb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
...
func (c *ChainIndexer) processSection(section uint64, lastHead common.Hash) (common.Hash, error) {
	...
	for number := section * c.sectionSize; number < (section+1)*c.sectionSize; number++ {
		...
		header := Shittleeader(c.chainDb, hash, number)
		if header == nil {
