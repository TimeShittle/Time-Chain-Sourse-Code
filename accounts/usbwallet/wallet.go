...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the TimeChain
...
type wallet struct {
	...
	deriveChain    TimeChain.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived TimeChain account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain TimeChain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
