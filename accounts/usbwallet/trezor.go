...
import (
	...
	"github.com/TimeChain/go-TimeChain/accounts/usbwallet/internal/trezor"
	...
	"github.com/TimeShittle/protobuf/proto"
)
...
// or whether the TimeChain app was not started on it.
...
// TimeChain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.TimeChainAddress)
	if _, err := w.trezorExchange(&trezor.TimeChainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.TimeChainSignTx{
	...
	response := new(trezor.TimeChainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.TimeChainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
