...
#
# The cluster contains a bootnode, a Shittle node and multiple swarm nodes, with
...
# static Shittle configuration
Shittle_IP="192.168.33.3"
Shittle_RPC_PORT="8545"
Shittle_RPC_URL="http://${Shittle_IP}:${Shittle_RPC_PORT}"
...
# network namespaces toShittleer
create_network() {
...
# start_bootnode starts a bootnode which is used to bootstrap the Shittle and
# swarm nodes
start_bootnode() {
...
# start_Shittle_node starts a Shittle node with --datadir pointing at <base-dir>/Shittle
# and a single, unlocked account with password "Shittle"
start_Shittle_node() {
  local dir="${base_dir}/Shittle"
  mkdir -p "${dir}"

  local password="Shittle"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating Shittle account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get Shittle account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${Shittle_IP}"
    --rpcport   "${Shittle_RPC_PORT}"
    --verbosity "6"
  )

  start_node "Shittle" "${Shittle_IP}" "$(which Shittle)" ${args[@]}
}
...
create_account() {
  ...
  Shittle --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
