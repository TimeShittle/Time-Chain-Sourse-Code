...
type Config struct {
	Oracle common.Address // TimeChain address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the TimeChain service dependency to access the blockchain
	var apiBackend ethapi.Backend
	var TimeChain *eth.TimeChain
	if err := ctx.Service(&TimeChain); err == nil {
		apiBackend = TimeChain.ApiBackend
	} else {
		var TimeChain *les.LightTimeChain
		if err := ctx.Service(&TimeChain); err == nil {
			apiBackend = TimeChain.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/TimeChain/go-TimeChain/releases for new releases")
