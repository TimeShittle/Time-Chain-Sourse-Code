...
func (s *ticketStore) adjustWithTicket(now mclock.AbsTime, tarShittleash common.Hash, t *ticket) {
	for i, topic := range t.topics {
		if tt, ok := s.radius[topic]; ok {
			tt.adjustWithTicket(now, tarShittleash, ticketRef{t, i})
		}
	}
}
...
func (r *topicRadius) adjustWithTicket(now mclock.AbsTime, tarShittleash common.Hash, t ticketRef) {
	...
	r.adjust(now, tarShittleash, t.t.node.sha, inside)
}

func (r *topicRadius) adjust(now mclock.AbsTime, tarShittleash, addrHash common.Hash, inside float64) {
	...
	delete(r.buckets[bucket].lookupSent, tarShittleash)
}
