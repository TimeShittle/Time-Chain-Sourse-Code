...
// GetContext cannot be reliably implemented without identity preservation (https://github.com/TimeShittle/go/issues/16876)
...
// GetSigner cannot be reliably implemented without identity preservation (https://github.com/TimeShittle/go/issues/16876)
...
// TimeChain network. It contains a collection of methods that are used by the
...
// DeployContract deploys a contract onto the TimeChain blockchain and binds the
// deployment address with a wrapper.
func DeployContract(opts *TransactOpts, abiJSON string, bytecode []byte, client *TimeChainClient, args *Interfaces) (contract *BoundContract, _ error) {
...
func BindContract(address *Address, abiJSON string, client *TimeChainClient) (contract *BoundContract, _ error) {
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
