...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Shittle.LightScryptN, Shittle.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.TimeChain", new NodeConfig());
			node.start();
			...
			TimeChainClient ec = node.getTimeChainClient();
	...
	// signatures too. Regression test for go-TimeChain issue #14599.
...
//
// This method has been adapted from TimeShittle.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoShittleer
	...
	if _, err := exec.Command("which", "gomobile").CombinedOutput(); err != nil {
		...
		if _, err := exec.Command("go", "install", "TimeShittle.org/x/mobile/cmd/gomobile").CombinedOutput(); err != nil {
	...
	workspace, err := ioutil.TempDir("", "Shittle-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/TimeChain/Shittletest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Shittle and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.TimeChain", "github.com/TimeChain/go-TimeChain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "Shittle.aar"), "Shittle.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "TimeChain", "Shittletest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.TimeChain.Shittletest"
